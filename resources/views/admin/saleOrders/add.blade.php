...
@endsection
@section('content')
<div class="col-sm-12" id="sales-quotation-add-container">
  <form id="quotationForm" action='{{url("order/store")}}' method="post">
    {{ csrf_field() }}
            ...
            <div class="col-md-6">
              <div class="form-group row mb-0">
                ...
                <div class="col-md-7 p-md-0 pr-md-4" id="customer-list-in-quotation">
                  <select class="select2 invoice-customers" name="customer_id" id="customers">
                    <option value="">{{ __('Select One') }}</option>
                    @foreach($customerData as $data)
                      @if (isset($data->id) && isset($data->currency->id))
                        <option value="{{ $data->id }}" data-currency_id="{{ isset($data->currency->id) ?  $data->currency->id : ''}}"  data-symbol="{{ isset($data->currency->symbol) ? $data->currency->symbol : '' }}" data-name="{{ isset($data->currency->name) ? $data->currency->name : '' }}">{{ $data->first_name .' '. $data->last_name }} ({{ isset($data->currency->name) ?  $data->currency->name : '' }})</option>
                      @endif
                    @endforeach
                  </select>
                ...
                <div class="col-md-1 p-md-0" id="add-button-md">
                  <button class="btn btn-outline-primary custom-btn-small float-right m-0" type="button" data-remote="{{ url('pos/add-customer') }}?type=quotation" data-label="add-customer" data-toggle="modal" data-target="#theModal"><span class="feather icon-user-plus"></span></button>
                </div>
              ...
              <div class="form-group row">
                ...
                <div class="input-group date col-md-8 p-md-0">
                  <div class="input-group-prepend">
                    <span class="input-group-text">QN-</span>
                  </div>
            ...
            <div class="col-md-6">
              <div class="form-group row">
                ...
                <div class="inv-header-type col-md-8 p-md-0">
                  ...
                  @endif
                  <select name="invoice_type" id="inv-type" class="form-control select2" data-previous="quantity"  {{ $menu == 'project' ? 'disabled' : '' }} >
                    @if($menu != 'project')
        ...
        <div class="m-t-10 p-0">
          <div class="row">
            <div class="col-sm-8 mb-3">
              <div class="form-group row mb-0">
                <label class="col-md-2 col-form-label item-block">{{ __('Add') }}&nbsp;&nbsp;<span class="searchItemTh"> {{ __('Item') }} </span></label>
                ...
                <div class="inv-content" id="inv-content">
                      ...
                      <thead>
                        <tr class="tbl_header_color">
                          ...
                          <th class="itemPrice text-center w-10">
                            <span class="qtyPriceTh">{{ __('Price') }} (<span class="currencySymbol">{{ isset($default_currency->symbol) ? $default_currency->symbol : '' }}</span>)</span>
                            <span class="hourPriceTh">{{ __('Rate') }}(<span class="currencySymbol">{{ isset($default_currency->symbol) ? $default_currency->symbol : '' }}</span>)</span>
                          ...
                          <th class="itemAmount text-center w-10">{{ __('Amount') }} (<span class="currencySymbol">{{ isset($default_currency->symbol) ? $default_currency->symbol : '' }}</span>)</th>
                  ...
                  <div id="itemInputFooter" class="table-responsive">
                    <table>
                      ...
                      <tr>
                        <td>
                          <label id="customAmountDescription-error" class="error display_block" for="customAmountDescription"></label>
                        </td>
                        <td>
                          <label id="inputCustomAmount-error" class="error display_block" for="inputCustomAmount"></label>
                        </td>
                      </tr>
                      <tr class="grandTotal">
